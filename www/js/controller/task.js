...
app.controller('TaskBaseCtrl', function ($scope, ajax, userService) {
    ...
    $scope.commonPostAction = function(taskId, actionType, description, images, transferUser, cb) {
        var data = {};
        if (description){
...
app.controller('GrabTaskCtrl', function ($scope, userService, ajax) {
    $scope.postGrabAction = function($event, taskId) {
        ...
        var company = userService.getTaskCompanyId();
        if (!company){
            company = userService.getTaskCompanyId();
        }
        var data = {};
        $.notify.progressStart();
        ajax.post({
...
app.controller('TaskDetailCtrl', function ($scope, $location, $state, userService, platformService, $http, $timeout, $window, ajax, routerService) {
    ...
    function formatActions(historyList) {      // 将操作历史格式化成界面显示
        ...
        if (taskData.stage_id === TaskStatus.ToAccept) {
            initMembers();
        }
    }
                    ...
                    if (!userPicker){
                        ...
                        taskTypeButton.addEventListener('click', function(event) {
                            userPicker.show(function(items) {
                                ...
                                // $scope.$apply();
                                $scope.commonPostActionWithParams($scope.taskData.id, TaskAction.Transfer, {transfer_user: items[0].value}, function (data) {
                                    $scope.taskData = formatTaskStatusName(data);
                                    // 调用Android接口
                                    window.android && window.android.onJsCallbackForPrevPage('updateTask', JSON.stringify(data));
                                    formatTaskStatus();
...
app.controller('TaskTransferCtrl', function ($scope) {      //驳回关闭请求
